...
interface PrimaryLayoutProps {
  children?: ReactNode
  header?: ReactNode | ((props: { toggleSideBar: () => void }) => ReactNode)
  sidebar?: ReactNode | ((props: { sidebarOpen: boolean; toggleSideBar: () => void }) => ReactNode)
  footer?: ReactNode
}

export function PrimaryLayout({ children, header, sidebar, footer }: PrimaryLayoutProps) {
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-zinc-50/50 dark:bg-zinc-950/20">
      {typeof header === 'function' ? header({ toggleSideBar }) : header}
      ...
      >
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in duration-300 flex flex-col flex-1">
